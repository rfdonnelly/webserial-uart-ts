...
it<Ctxt>("write", ({ o }) => {
  expect(
    o.parse_request([
      0x47, 0x50, 0x87, 0x65, 0x43, 0x21, 0x12, 0x34, 0x56, 0x78, 0x55, 0x47,
    ]),
  ).toEqual([
    [0x47],
    {
      ...
      bytes: new Uint8Array([
        0x47, 0x50, 0x87, 0x65, 0x43, 0x21, 0x12, 0x34, 0x56, 0x78, 0x55,
      ]),
    },
  ]);
});
...
it<Ctxt>("read", ({ o }) => {
  expect(
    o.parse_request([0x47, 0x30, 0x12, 0x34, 0x56, 0x78, 0x55, 0x47]),
  ).toEqual([
    [0x47],
    {
      command: "Read",
...
it<Ctxt>("multiple", ({ o }) => {
  expect(
    o.parse_requests([
      0x47, 0x50, 0x87, 0x65, 0x43, 0x21, 0x12, 0x34, 0x56, 0x78, 0x55, 0x47,
      0x30, 0x12, 0x34, 0x56, 0x78, 0x55, 0x47,
    ]),
  ).toEqual([
    [0x47],
    [
...
it<Ctxt>("stream", async ({ o }) => {
  const byte_stream = new ReadableStream({
    start(controller) {
      ...
      controller.enqueue([0x87, 0x65, 0x43, 0x21]);
      controller.enqueue([0x12, 0x34, 0x56, 0x78]);
