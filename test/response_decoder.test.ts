...
it<Ctxt>("write", ({ o }) => {
  expect(o.parse_response([0x47, 0x50, 0x55, 0x47])).toEqual([
    [0x47],
...
it<Ctxt>("read", ({ o }) => {
  expect(
    o.parse_response([0x47, 0x30, 0x78, 0x56, 0x34, 0x12, 0x55, 0x47]),
  ).toEqual([
    [0x47],
    {
      command: "Read",
...
it<Ctxt>("incomplete", ({ o }) => {
  expect(o.parse_response([0x47])).toEqual([[0x47], "Incomplete"]);
});

it<Ctxt>("none", ({ o }) => {
  expect(o.parse_response([0x00])).toEqual([[0x00], "None"]);
});

it<Ctxt>("bad", ({ o }) => {
  expect(o.parse_response([0x47, 0x00, 0x47])).toEqual([[0x47], "Bad"]);
});

it<Ctxt>("multiple", ({ o }) => {
  expect(
    o.parse_responses([
      0x47, 0x50, 0x55, 0x47, 0x30, 0x78, 0x56, 0x34, 0x12, 0x55, 0x47,
    ]),
  ).toEqual([
    [0x47],
    [
      {
        command: "Write",
        crc: 0x55,
        bytes: new Uint8Array([0x47, 0x50, 0x55]),
      },
      {
        command: "Read",
        data: 0x12345678,
